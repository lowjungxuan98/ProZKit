...
class ProZoomController: NSObject {
    static let shared = ProZoomController()
...
extension ProZoomController: MobileRTCCustomizedUIMeetingDelegate {
    func onInitMeetingView() {
        ...
        if let waitVC = navigationController?.presentedViewController as? CustomMeetingViewController {
            return
        }
...
extension ProZoomController: MobileRTCWaitingRoomServiceDelegate {
    func onWaitingRoomPresetAudioStatusChanged(_ audioCanTurnOn: Bool) {
        PrettyLogger.log("[MEETING PRESET AUDIO: \(audioCanTurnOn)]")
    }
    
    func onWaitingRoomPresetVideoStatusChanged(_ videoCanTurnOn: Bool) {
        PrettyLogger.log("[MEETING PRESET VIDEO: \(videoCanTurnOn)]")
    }
    
    func onCustomWaitingRoomDataUpdated(_ data: MobileRTCCustomWaitingRoomData?) {
        PrettyLogger.log("Custom Waiting Room Data Updated: \(data?.description)")
    }
}


class WaitingHostViewController: UIViewController {
    private lazy var waitingHostView: UILabel = {
        let view = UILabel()
        view.text = "Waiting Host"
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        view.addSubview(waitingHostView)
        NSLayoutConstraint.activate([
            waitingHostView.centerXAnchor.constraint(equalTo: view.centerXAnchor),
            waitingHostView.centerYAnchor.constraint(equalTo: view.centerYAnchor)
        ])
    }
}
